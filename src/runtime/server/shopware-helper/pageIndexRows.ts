...
import type { PageIndexRow } from '@laioutr-core/core-types/orchestr';
 ...
 * can't be dereferenced by the consumers that read `PageIndexRow.meta.previewImage` (Cockpit picker,
 * sitemaps, indexers), which all live outside the Nuxt image pipeline.
 */
export const toProductPageMeta = (product: ShopwareProduct): PageIndexRow['meta'] => ({
  title: swTranslated(product, 'name') ?? product.id,
  previewImage: product.cover?.media?.url,
  lastModified: product.updatedAt ?? undefined,
});

export const toProductPageRow = (product: ShopwareProduct): PageIndexRow => ({
